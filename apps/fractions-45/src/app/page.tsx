...
/** Fraction Workshop — Grade Band: 4-5 */
export default function Fractions45App() {
  ...
  return (
    <div style={{ minHeight: "100vh", fontFamily: "system-ui", background: "#F0F5F7" }}>
      <header style={{ padding: "16px 24px", background: "#fff", borderBottom: "1px solid #E8E8E8" }}>
        <h1 style={{ margin: 0, color: "#C4743A" }}>{manifest.name}</h1>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {(["concrete", "representational", "abstract"] as CRALayer[]).map(layer => (
            <button key={layer} onClick={() => setCurrentLayer(layer)}
              style={{ padding: "8px 16px", border: currentLayer === layer ? "2px solid #C4743A" : "1px solid #ccc",
                background: currentLayer === layer ? "#C4743A" : "#fff",
